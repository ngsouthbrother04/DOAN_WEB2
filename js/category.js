...
window.addEventListener('load', () => {
  if (!category) {
    document.getElementById('product-list').innerHTML = '<p>Không có danh mục được chọn.</p>';
    return;
  }


  document.querySelector('.product-section h2').textContent = `Sản phẩm - ${category.replace('-', ' ').toUpperCase()}`;
